...
unsigned long next_token_len(char *uri, unsigned long skip_pos) {
  char *str_copy = malloc(strlen(uri) + 1);
  strcpy(str_copy, uri);
...
int main(void) {
  ...
  char* payload_prefix = EXAMPLE_PASS;
  char* version_identifier = EXAMPLE_PASS + token1_len + 1;
  char* base32_encoded_cwt = EXAMPLE_PASS + token1_len + 1 + token2_len + 1;
